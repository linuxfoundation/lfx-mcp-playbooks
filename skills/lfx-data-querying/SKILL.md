...
| How many meetings a company's people attended over a period | SQL assistant, over the attendance data, because the occurrences metric carries no organisation | distinct occurrences with an attendee from the company; never attendances, never attendances divided by people; a layer grouping by meeting name and day is the check, not the figure (the `lfx-deck-building` skill's org-briefing reference, Section 5) | generated SQL, interim |
...
- **A check is a computed figure.** A sum that checks a breakdown
  ...
  "Roughly" is not a check. A breakdown cut by a limit is a floor and
  says so; a base derived as the total minus the unattributed row is
  described as derived, not as a sum of the rows.
- **A roll-forward starts on 31 December of the prior year.** Start plus
  new minus churned against the end never balances exactly, and it is
...
16. Per-project maintainer counts include people the roster inherits
    ...
    two projects counts once in each. Said with every per-project
    figure. A figure that uses the family's split (by role, by source,
    or a split column) names it and shows it beside the total it was
    cut from; a split alone reads as a total:
